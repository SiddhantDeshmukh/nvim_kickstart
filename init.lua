-- Config Owner: Siddhant A. Deshmukh
...
--    :Lazy update
--
-- Disable netrw so that neo-tree (later down) is used by default
vim.g.loaded_netrw = 1
vim.g.loaded_netrwPlugin = 1

-- Create empty pane at bottom right for R plots
-- vim.g.R_after_start = { ':winc j', ':vsplit', ':vertical resize ' .. math.floor(vim.fn.winwidth(0) / 3), ':winc k' }
-- Place object browser above plot plane
...
require('lazy').setup({
    ...
    config = function()
      vim.api.nvim_create_autocmd('LspAttach', {
        ...
        callback = function(event)
          ...
          -- The following code creates a keymap to toggle inlay hints in yourminima
      ...
      local servers = {
        clangd = {},
        -- gdtoolkit = {
        --   cmd = { 'godot', '--lsp', '--port', '6005', '--host', '127.0.1.1' },
  ...
  -- Harpoon
  {
    ...
    config = function()
      ...
      -- Jump to specific files (disabled for now)
  ...
  -- Neotree with the <leader>ee explorer keybind
  {
    ...
    lazy = false, -- we want to use this by default, so don't lazy load
    keys = {
      {
        '<leader>ee',
        function()
